...
import { mujiProfile } from '../../data/muji';

// 인프라 직무 기준으로 스킬 노출 순서만 바꾼다. 항목 자체는 profile.js 그대로.
const ORDER = ['Backend / Infra', 'Language', 'AI / ML', 'Tools', 'AI Tools'];
const orderedSkills = [...profile.skills].sort(
  (a, b) => ORDER.indexOf(a.category) - ORDER.indexOf(b.category)
);

export default function MujiProfilePage() {
  return (
    <div className="flex min-h-screen print:min-h-0 print:h-auto bg-white">
      <aside className="w-72 bg-slate-900 text-white flex flex-col gap-6 p-8 shrink-0 print:w-56 print:p-5 print:gap-4">
        <div className="flex flex-col items-center gap-3 print:gap-2">
          ...
          <div className="text-center">
            ...
            <p className="text-blue-300 font-medium mt-1 text-sm print:text-xs">{mujiProfile.title}</p>
          </div>
        </div>

        <p className="text-slate-300 text-xs leading-relaxed border-t border-slate-700 pt-4 print:pt-3">
          {mujiProfile.bio}
        </p>
        ...
        <div className="border-t border-slate-700 pt-4 print:pt-3">
          ...
          <div className="flex flex-col gap-1.5">
            <a href={`mailto:${profile.email}`} className="text-xs text-slate-300">✉ {profile.email}</a>
            <span className="text-xs text-slate-300">☎ {profile.phone}</span>
            <a href={profile.github} className="text-xs text-slate-300" target="_blank" rel="noreferrer">
      ...
      <main className="flex-1 p-8 print:p-6 flex flex-col gap-6 print:gap-4">
        <div className="border-l-4 border-blue-500 pl-4 py-1">
          <p className="text-slate-700 font-medium text-sm print:text-xs">{mujiProfile.headline}</p>
        </div>
