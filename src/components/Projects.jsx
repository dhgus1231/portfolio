      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-col gap-6 print:block print:space-y-6">
          {profile.projects.map((project) => (
            ...
            >
              {/* Header */}
              <div className={`px-6 py-4 ${project.highlight ? 'bg-blue-50' : 'bg-slate-50'}`}>
              ...
              <div className="px-6 py-4 bg-white flex flex-col gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-700 leading-relaxed">{project.overview}</p>
                </div>

                {/* Contribution */}
                <div>
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">My Contribution</p>
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {project.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-xs px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 font-medium print:border print:border-slate-300 print:bg-white"
                    >
                      {tag}
                    </span>
                  ))}
